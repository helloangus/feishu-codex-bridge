    ...
    tokio::time::timeout(Duration::from_secs(20), async {
        ...
        for round in 0..2 {
            let mut server = AppServer::spawn(
                Path::new(env!("CARGO_BIN_EXE_fake-codex-runtime")),
                &["directory".into()],
                &root,
                27,
            )
            ...
            let source = tokio::spawn(async move {
                loop {
                    let event =
                        tokio::select! {_=stop.cancelled()=>break,event=server.next_event()=>event};
            ...
            if round == 0 {
                ...
                send(&tx, "wrong-user", "other", &command).await?;
                until(&mut output, "创建确认无效").await?;
                let (ack, wait) = oneshot::channel();
                tx.send(Input {
                    attachments: vec![],
                    card: None,
                ...
                until(&mut output, "创建确认无效").await?;
                assert!(!root.join("created").exists());
                send(&tx, "confirm", "owner", &command).await?;
                until(&mut output, "目录已创建并切换").await?;
                assert!(root.join("created/nested").is_dir());
                send(&tx, "confirm-repeat", "owner", &command).await?;
                until(&mut output, "创建确认无效").await?;
                send(
                    &tx,
                    "return-root",
                ...
                until(&mut output, "已切换目录").await?;
                send(&tx, "old-confirm", "owner", &abandoned).await?;
                until(&mut output, "创建确认无效").await?;
                assert!(!root.join("abandoned").exists());
                send(&tx, "model", "owner", "/model chosen").await?;
                until(&mut output, "设置已保存").await?;
                send(&tx, "plan", "owner", "/plan on").await?;
                until(&mut output, "设置已保存").await?;
                send(&tx, "child-turn", "owner", "hello").await?;
                assert!(
                    until(&mut output, "执行完成")
                        .await?
                ...
                send(&tx, "back", "owner", &format!("/cd {}", child.display())).await?;
                until(&mut output, "已切换目录").await?;
                send(&tx, "busy-proposal", "owner", "/cd after-task").await?;
                let after_task = confirmation(&until(&mut output, "目录不存在").await?)?;
                send(&tx, "hold", "owner", "hold").await?;
                until(&mut output, "已开始执行").await?;
                while !tokio::fs::try_exists(root.join("holding")).await? {
                    tokio::time::sleep(Duration::from_millis(5)).await;
                }
                send(&tx, "busy-cd", "owner", "/cd ..").await?;
                until(&mut output, "有任务执行中").await?;
                send(&tx, "busy-confirm", "owner", &after_task).await?;
                until(&mut output, "有任务执行中").await?;
                send(&tx, "queued", "owner", "must be cancelled").await?;
                until(&mut output, "请求已接收").await?;
                send(&tx, "other-cd", "other", "/cd child space").await?;
                until(&mut output, "有任务执行中").await?;
                send(&tx, "stop", "owner", "/stop").await?;
                until(&mut output, "任务已停止").await?;
                send(&tx, "idle-confirm", "owner", &after_task).await?;
                until(&mut output, "目录已创建并切换").await?;
                send(
                    &tx,
                    "return-child",
                    "owner",
                    &format!("/cd {}", child.display()),
                )
                .await?;
                until(&mut output, "已切换目录").await?;
                ...
                send(&tx, "restart-confirm", "owner", &restart_confirmation).await?;
                until(&mut output, "创建确认无效").await?;
