...
本入口运行 Rust 业务、Codex 适配与 REST；Python 仅负责飞书 SDK 长连接。它支持授权文本任务、串行排队、会话连续、文本回复，以及 `/help`、`/status`、`/stop`、`/new`。这是前台最小版，不是原始计划中全部功能完成的过渡版。
...
- 队列最多 64 项（含认领保存中），文本任务与 `/new` 共用最近 1,000 条持久化消息去重；其他控制命令只有运行期去重，重投可能再次显示提示，停止本身幂等。`/new` 先认领再清除绑定，认领后崩溃或保存失败不自动重试；收到失败提示时需重新发送一条 `/new`。去重窗口外的历史命令不保证被识别。
