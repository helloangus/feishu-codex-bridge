...
当前 Rust 二进制是**离线迁移和配置检查工具**，还不是生产机器人。
...
| P3 | Rust 主业务 + Python SDK 长连接薄进程 | 已实现 REST/卡片/流式附件接口、SDK 薄进程和版本化 IPC 解码；已补 IPC 接受确认和连接生命周期；主循环耐久认领接入、REST 完整契约测试和独立机器人验收待完成。 |
