...
当前 Rust 二进制已实现原生长连接、业务交互、附件成果、配对与两层服务监督，详见 [运行说明](minimal-runtime.md)。新增配置生成、部署包装与本机打包入口见 [部署指南](rust-deployment.md)，本批待统一验证，尚未完成生产验收。
...
| P2 | 完整 Codex 进程/RPC/通知/审批适配 | 已实现有界双向 RPC、连接代次、子进程初始化/回收、异步 backend、0.153.4 schema 与 turn fixture；已补输出/Plan/完成/归档和基础审批/问答类型映射，执行循环与额外权限适配已实现，真实协议交互仍需验收。 |
| P3 | Rust 主业务 + Python SDK 长连接薄进程 | 最小版、会话恢复与模型/Plan 单用户真实验收通过；归档、压缩、目录、交互卡片、审批、问答、附件成果、Plan 确认/实施以及 Markdown 回复/流式预览已开发。新增功能真实验收延后；服务生命周期已实现。 |
| P4 | Rust 飞书 WebSocket | 原生地址发现、Protobuf、EVENT/CARD、分片、心跳、重连与代理已接入 bootstrap；19 项连接相关离线测试覆盖 WebSocket、重连、代理、TLS 与 CLI 配置。真实飞书、代理及 Termux/手机验收延后，尚未部署，详见 PLAN.md。 |
| P5 | 运维、发布、回退与退役 | 两层 supervisor、健康控制通道已实现；配置、部署和本机打包入口待本批验证。发布产物实际构建、72 小时持续运行和生产回退演练待完成。 |
...
接收附件、当前线程生成图片及上传生命周期已通过 `bridge-local::delivery` 接入。应用层通过注入的消息端口启动准备、线程绑定和交付；路径访问采用 no-follow，资源用私有临时文件暂存。任务前后快照、内容去重、失败汇总和下一任务串行门禁见 `docs/minimal-runtime.md`。真实飞书验收延后。
